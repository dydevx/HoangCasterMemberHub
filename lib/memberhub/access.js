import { routePathFor } from "@/lib/memberhub/slug";
...
export function dashboardPathFor(user, data = null) {
  ...
  if (role === "store_owner") {
    ...
    return routePathFor(shop?.slug || shop?.name) || "/";
  }

  const customer = data?.customers?.[0];
  const shop = data?.shops?.find((item) => item.id === customer?.shop_id) || data?.shops?.[0];

  const customerPath = routePathFor(shop?.slug || shop?.name, customer?.slug || customer?.name);
  if (customerPath) {
    return customerPath;
  }
