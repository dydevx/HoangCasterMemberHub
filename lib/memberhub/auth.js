...
function tokenSecret() {
  const secret = process.env.MEMBERHUB_AUTH_SECRET || process.env.SUPABASE_SERVICE_ROLE_KEY;
  if (!secret || secret.length < 32) {
    throw new Error("A server-side authentication secret must contain at least 32 characters");
  }
  return secret;
}
